...
import {
  encryptSerpent256ECB,
  decryptSerpent256ECB,
} from "@/app/cryptographicPrimitives/serpent";
...
export const silentlyDecryptDataWithTwoCiphersCBC = async (
  ...
  const derivedKey = await derive416BytesUsingArgon2id(
    password,
    salt,
    iterations,
  );
  ...
  for (let i = 48; i < dataLengthNoLC; i += chunkSize) {
    ...
    const xorChunk = decryptedChunk.map(
      (byte, index) => byte ^ previousCiphertext[index],
    );
  ...
  while (streamCipherOffset < decryptedDataUint8Array.length) {
    const input = Array.from(chacha20key)
    ...
    const chunk = decryptedDataUint8Array.slice(
      streamCipherOffset,
      Math.min(
        streamCipherOffset + chunkSizeForStreamCipher,
        decryptedDataUint8Array.length,
      ),
    );
    const nonce = chacha20key.slice(32, 40);
    const chacha20 = new ChaCha20();
    const decryptedChunk = chacha20.decrypt(
      chacha20key.slice(0, 32),
      chunk,
      nonce,
    );
  ...
  const newTag = await computeTagForDataUsingHMACSHA512(
    hmacKey,
    decryptedWithStreamCipher,
  );
...
export const silentlyEncryptDataWithTwoCiphersCBCnoPadding = async (
  ...
  while (offset < totalSize) {
    ...
    const chunk = tag_and_data.slice(
      offset,
      Math.min(offset + chunkSize, totalSize),
    );
    const nonce = chacha20key.slice(32, 40);
    const chacha20 = new ChaCha20();
    const encryptedChunk = chacha20.encrypt(
      chacha20key.slice(0, 32),
      chunk,
      nonce,
    );
...
export const silentlyEncryptDataWithTwoCiphersCBC = async (
  ...
  if (!paddingApplied) {
    ...
    const xorChunk = paddedChunk.map(
      (byte, index) => byte ^ previousCiphertext[index],
    );
    const encryptedChunk = await encryptSerpent256ECB(xorChunk, blockCipherKey);
    encryptedChunks.push(encryptedChunk);
...
export const computeTagForDataUsingHMACSHA512 = async (
  key: Uint8Array,
  data: Uint8Array,
) => {
...
const derive416BytesUsingArgon2id = async (
  password: Uint8Array,
  salt: Uint8Array,
  iterations: number,
) => {
...
export const deriveBytesUsingArgon2id = async (
  password: Uint8Array,
  salt: Uint8Array,
  iterations: number,
...
export const decryptFieldValueWithTwoCiphersCBCnoPadding = async (
  ...
  const decryptedLastBlock = await decryptSerpent256ECB(
    encryptedLastBlock,
    blockCipherKey,
  );
  const decryptedLastBlockXORed = decryptedLastBlock.map(
    (byte, index) => byte ^ previousCiphertext[index],
  );
...
export const decryptFieldValueWithTwoCiphersCBC = async (
  ...
  if (paddingLength === 0) {
    ...
    const unpaddedLastBlock = decryptedLastBlockXORed.slice(
      0,
      16 - paddingLength,
    );
...
export const CheckRecordIntegrity = async (
  ...
  const computedTag = await computeTagForDataUsingHMACSHA512(
    hmacKey,
    plaintextToVerify,
  );
