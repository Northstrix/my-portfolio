...
const WorkExperienceCardContent: React.FC<WorkExperienceCardContentProps> = ({
  ...
  return (
    <div>
      <div className="font-sans font-bold text-[var(--foreground)] mb-2" style={{ lineHeight: "1.25", marginBottom: "16px", fontSize: `${titleSize}px`, textAlign: resolvedTextAlign }}>
        {t(position)}
      </div>
      ...
      <ul style={{ textAlign: resolvedTextAlign, listStyleType: "none", padding: 0 }}>
        {skills.map((skill, index) => (
          <li key={index} className="flex items-center mb-2" style={{ fontSize: `${descriptionSize}px` }}>
            <IconCheck stroke={2.5} size={descriptionSize} style={{ marginRight: '0.5rem', marginLeft: isRTL ? '0.5rem' : 0 }} />
            <span>{t(skill)}</span>
          </li>
        ))}
      </ul>
...
const WorkExperienceContent: React.FC<WorkExperienceContentProps> = ({ isRTL }) => {
    ...
    <div ref={componentRef} style={{ display: 'flex', flexDirection: 'column', gap: '0px', justifyContent: 'flex-start', alignItems: 'stretch', width: '100%', maxWidth: maxSectionWidth }}>
      ...
      <StructuredBlock {...addRTLProps(contentProps, isRTL)}>
        <div className="items-center justify-center relative flex" ref={cardsContainerRef}>
          <div style={{ width: '100%' }} className="grid grid-cols-1 gap-4 max-w-full mx-auto">
            <WorkExperienceCard
