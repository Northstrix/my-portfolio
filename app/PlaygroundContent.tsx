...
const PlaygroundContent: React.FC<PlaygroundContentProps> = ({ isRTL }) => {
  ...
  const reversedTextOptions = textOptions.map((option) =>
    [...option].reverse(),
  );
  ...
  useEffect(() => {
    const updateText = () => {
      ...
      setSecondCardText(
        isHebrew ? reversedTextOptions[activeCard] : textOptions[activeCard],
      );
    };
    updateText();
  ...
  const showRectangleNotification = (
    type: "success" | "error",
    message: string,
    message1?: string,
  ) => {
  ...
  const handleCardClick = (index?: number) => {
    ...
    if (action <= 74) {
      ...
      if (activeCard === 0) {
        ...
        const currentIndex = textOptions.findIndex(
          (opt) => JSON.stringify(opt) === JSON.stringify(secondCardText),
        );
        let newIndex;
        do {
        ...
        setSecondCardText(
          i18n.language === "he"
            ? reversedTextOptions[newIndex]
            : textOptions[newIndex],
        );
      }
    } else if (action <= 208) {
      const currentIndex = textOptions.findIndex(
      ...
      const toastMessage =
        randomLang === 0
          ...
          : `${quote.hebrewQuote} - ${quote.hebrewAuthor}`;
  ...
  useEffect(() => {
    const updateTextSize = () => {
      if (containerRef.current) {
        ...
        const size =
          activeInscription.minSize +
          ((activeInscription.maxSize - activeInscription.minSize) *
            Math.min(
              Math.max(width - activeInscription.minWidth, 0),
              activeInscription.maxWidth - activeInscription.minWidth,
            )) /
            (activeInscription.maxWidth - activeInscription.minWidth);
        setTextSize(size);
      }
  ...
  const textShadow = swapColors
    ? `-1px -1px 0 var(--card-background), 1px -1px 0 var(--card-background), -1px 1px 0 var(--card-background), 1px 1px 0 var(--card-background)`
    : `-1px -1px 0 var(--refresh-inscription-color), 1px -1px 0 var(--refresh-inscription-color), -1px 1px 0 var(--refresh-inscription-color), 1px 1px 0 var(--refresh-inscription-color)`;
  const oneOfThree = [INSCRIPTION, INSCRIPTION2, INSCRIPTION3].includes(
    activeInscription,
  );
      ...
      >
        ...
        <StructuredBlock {...addRTLProps(contentProps, isRTL)}>
          <div
            style={{
              width: "100%",
              height: isMobileDevice ? "auto" : "calc(100vh - 169px)",
              minHeight: isMobileDevice ? "auto" : "calc(100vh - 169px)",
