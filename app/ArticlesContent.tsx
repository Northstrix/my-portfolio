...
const NuanceIconContainer = styled.div<{
  backgroundColor?: string;
  size?: string;
}>`
...
const NuanceIcon: React.FC<NuanceIconProps> = ({ size, backgroundColor }) => {
  return (
    <NuanceIconContainer backgroundColor={backgroundColor} size={size}>
      <NuanceIconImage
        src="https://nuance.xyz/assets/images/loaders/nuance-loader.gif"
        alt="Nuance Logo"
      />
    </NuanceIconContainer>
  );
...
const ArticlesContent: React.FC<ArticlesContentProps> = ({
  ...
  const [metaFontSizeFirst, setMetaFontSizeFirst] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxMetaFontSize,
  );
  const [authorImageSizeFirst, setAuthorImageSizeFirst] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxAuthorImageSize,
  );
  const [socialIconSizeFirst, setSocialIconSizeFirst] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxSocialIconSize,
  );
  const [titleSizeFirst, setTitleSizeFirst] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxTitleSize,
  );
  const [tagTextSizeFirst, setTagTextSizeFirst] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxTagTextSize,
  );
  const [metaFontSizeSecond, setMetaFontSizeSecond] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxMetaFontSize,
  );
  const [authorImageSizeSecond, setAuthorImageSizeSecond] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxAuthorImageSize,
  );
  const [socialIconSizeSecond, setSocialIconSizeSecond] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxSocialIconSize,
  );
  const [titleSizeSecond, setTitleSizeSecond] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxTitleSize,
  );
  const [tagTextSizeSecond, setTagTextSizeSecond] = useState(
    TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop.maxTagTextSize,
  );
  ...
  const handleResize = useCallback(() => {
    if (blogPostsRef.current) {
      ...
      const sizes = isMobile
        ? TEXT_SIZES[isRTL ? "rtl" : "ltr"].mobile
        : TEXT_SIZES[isRTL ? "rtl" : "ltr"].desktop;
      const {
        minWidth,
        maxWidth,
        minMetaFontSize,
      ...
      const calculatedMetaFontSize =
        ((maxMetaFontSize - minMetaFontSize) / (maxWidth - minWidth)) *
          (width - minWidth) +
        minMetaFontSize;
      const calculatedAuthorImageSize =
        ((maxAuthorImageSize - minAuthorImageSize) / (maxWidth - minWidth)) *
          (width - minWidth) +
        minAuthorImageSize;
      const calculatedSocialIconSize =
        ((maxSocialIconSize - minSocialIconSize) / (maxWidth - minWidth)) *
          (width - minWidth) +
        minSocialIconSize;
      const calculatedTitleSize =
        ((maxTitleSize - minTitleSize) / (maxWidth - minWidth)) *
          (width - minWidth) +
        minTitleSize;
      const calculatedTagTextSize =
        ((maxTagTextSize - minTagTextSize) / (maxWidth - minWidth)) *
          (width - minWidth) +
        minTagTextSize;

      setMetaFontSizeFirst(Math.min(calculatedMetaFontSize, maxMetaFontSize));
      setAuthorImageSizeFirst(
        Math.min(calculatedAuthorImageSize, maxAuthorImageSize),
      );
      setSocialIconSizeFirst(
        Math.min(calculatedSocialIconSize, maxSocialIconSize),
      );
      ...
      setAuthorImageSizeSecond(
        Math.min(calculatedAuthorImageSize, maxAuthorImageSize),
      );
      setSocialIconSizeSecond(
        Math.min(calculatedSocialIconSize, maxSocialIconSize),
      );
      ...
      <StructuredBlock {...addRTLProps(contentProps, isRTL)}>
        <div ref={blogPostsRef}>
          <BlogPostHeader
            ...
            titleText={
              isRTL
                ? "איך בניתי אפליקציית ווב ללא ניסיון בג'אווהסקריפט"
            ...
            socialIcons={[
              {
                ...
                icon: (
                  <FontAwesomeIcon
                    icon={faMedium}
                    size={`${socialIconSizeFirst}px` as any}
                  />
                ),
            ...
            socialIcons={[
              {
                ...
                icon: (
                  <FontAwesomeIcon
                    icon={faMedium}
                    size={`${socialIconSizeSecond}px` as any}
                  />
                ),
                link: "https://medium.com/@Northstrix/diy-addressable-rgb-led-strip-controller-7ec61abfaf5e",
                backgroundColor: "var(--foreground)",
                foregroundColor: "var(--background)",
              },
              {
                ...
                link: "https://www.instructables.com/DIY-Addressable-RGB-LED-Strip-Controller/",
