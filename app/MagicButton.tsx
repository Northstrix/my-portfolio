...
const MagicButton: React.FC<Props> = ({ onButtonClick }) => {
  useEffect(() => {
    const particles = document.querySelectorAll(`.${styles.particle}`);
    particles.forEach(P => {
      const random = (min: number, max: number) => Math.floor(Math.random() * (max - min + 1) + min);
      (P as HTMLElement).style.setProperty('--x', `${random(20, 80)}`);
      (P as HTMLElement).style.setProperty('--y', `${random(20, 80)}`);
      (P as HTMLElement).style.setProperty('--duration', `${random(6, 20)}`);
      (P as HTMLElement).style.setProperty('--delay', `${random(1, 10)}`);
      (P as HTMLElement).style.setProperty('--alpha', `${random(40, 90) / 100}`);
      (P as HTMLElement).style.setProperty('--origin-x', `${Math.random() > 0.5 ? random(300, 800) * -1 : random(300, 800)}%`);
      (P as HTMLElement).style.setProperty('--origin-y', `${Math.random() > 0.5 ? random(300, 800) * -1 : random(300, 800)}%`);
      (P as HTMLElement).style.setProperty('--size', `${random(40, 90) / 100}`);
    });
  }, []);
  ...
  return (
    <div className={styles['sparkle-button']}>
      <button onClick={onButtonClick}>
        ...
        <svg className={styles.sparkle} viewBox="0 0 24 24" fill="none">
        ...
        <span className={styles.text}>Magic Button</span>
      </button>
      <div className={styles.bodydrop}></div>
      <span aria-hidden="true" className={styles['particle-pen']}>
        {[...Array(20)].map((_, index) => (
          <svg key={index} className={styles.particle} viewBox="0 0 15 15" fill="none">
