...
void main() {
    ...
    // 1. Make cells light (White background, subtracting the pattern)
    // Original was: vec3 col = c.x*(0.5 + 0.5*sin(64.0*c.x))*vec3(1.0);
    vec3 col = vec3(1.0) - c.x*(0.5 + 0.5*sin(64.0*c.x))*vec3(0.5);

    // Define Bright Orange Color
    vec3 orangeColor = vec3(1.0, 0.45, 0.0);

    // 2. Borders (Bright Orange)
    col = mix( orangeColor, col, smoothstep( 0.04, 0.07, c.x ) );

    // 3. Feature points (Bright Orange)
    float dd = length( c.yz );
    col = mix( orangeColor, col, smoothstep( 0.0, 0.12, dd) );
    col += orangeColor*(1.0-smoothstep( 0.0, 0.04, dd));
