...
const PlaygroundCardArea: React.FC<PlaygroundCardAreaProps> = ({ 
  ...
  const cards = [ 
    { 
      ...
      props: { 
        ...
        adjacentColor: firstCardRevealCanvas ? 'var(--background-adjacent-color)' : 'var(--refresh-inscription-color)', 
        foregroundColor: '#000', 
        imageHeightPercentage: 70, 
        imageSrc: '/playground-card-image.webp', 
        imageAlt: '', 
        outlineColor: 'var(--refresh-inscription-color)', 
        hoverOutlineColor: firstCardRevealCanvas ? 'var(--lightened-background-adjacent-color)' : 'var(--secondary-foreground)', 
        textArray: ['洪', '秀', '全'],
        minWidth: 200, 
        maxWidth: 400, 
        minTextSize: 16, 
  ...
  return ( 
    <div className="flex flex-col items-center w-full" style={{ maxWidth: '512px' }}> 
      <div className="relative w-full max-w-[412px] mx-auto" style={{ aspectRatio: '3/4' }}> 
        <AnimatePresence> 
          {cards.map((card, index) => ( 
            <motion.div 
              ...
              animate={{ 
                ...
                rotate: 0, 
                zIndex: index === activeCard ? 30 : hoveredCard === index ? 25 : 20 - index, 
